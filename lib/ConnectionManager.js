...
ConnectionManager.prototype.updateTimeout = function (sessionTimeout) {
    ...
    // Default the request timeout to the ping timeout if not set
    var defaultRequestTimeout = this.pingTimeout * 2;

    if (this.options.requestTimeout === null) {
        this.requestTimeout = defaultRequestTimeout;
    } else if (this.options.requestTimeout === false) {
        this.requestTimeout = false;
    } else {
        var number = Number(this.options.requestTimeout);
...
ConnectionManager.prototype.onSocketData = function (buffer) {
    ...
    if (self.state === STATES.CONNECTING) {
        ...
        default:
            ...
            // Track the response header having completed being read
            // for request timeout tracking
            pendingPacket.responseHeader = responseHeader;
...
ConnectionManager.prototype.onPacketQueueReadable = function () {
    ...
    while ((packet = this.packetQueue.shift()) !== undefined) {
        ...
        if (header !== null &&
            ...
            if (this.requestTimeout !== false) {
                this.ensurePacketResponse(packet, header);
            }
        }
...
ConnectionManager.prototype.ensurePacketResponse = function (packet, header) {
    var self = this;
    ...
    case jute.XID_NOTIFICATION:
        break;
    default:
        // About to write, ensure we timeout this request on failure
        setTimeout(function () {
            // If no response header then this packet never got a response
            if (packet.responseHeader === undefined) {
                // TODO, better error handling and logging need to be done.
                switch (self.state) {
                case STATES.CONNECTED:
                case STATES.CONNECTED_READ_ONLY:
                    // Fail the packet delivery
                    if (packet.callback) {
                        packet.callback(Exception.create(Exception.REQUEST_TIMED_OUT));

                        var index = self.pendingQueue.indexOf(packet);
                        if (index !== -1) {
                            self.pendingQueue.splice(index, 1);
                        }
                    }

                    // This will tear down and trigger reconnect
                    self.socket.destroy();
                    break;
                }
            }
