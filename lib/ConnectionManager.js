...
function Request(header, record) {
    this.header = header;
    this.record = record;
}

Request.prototype.byteLength = function () {
    var size = 0; // Start with 32bit integer length.
...
Request.prototype.toBuffer = function () {
    ...
    if (this.record) {
        offset += this.record.serialize(buffer, offset);
    }
            ...
            async.series([
                ...
                function (next) {
                    ...
                    socket.on('connect', function () {
                        ...
                        request = new Request(null, connReq);
