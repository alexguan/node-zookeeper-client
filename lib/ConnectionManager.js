...
ConnectionManager.prototype.onSocketData = function (buffer) {
    ...
    if (!isConnected(self)) {
        ...
        connectResponse.deserialize(buffer);
        ...
        default:
            ...
            if (pendingPacket.response.header.err === 0) {
                if (pendingPacket.request.header.type === jute.OP_CODES.GET_CHILDREN2) {
                    pendingPacket.callback(
                        null,
                        pendingPacket.response.payload.children,
                        pendingPacket.response.payload.stat
                    );
                } else if (pendingPacket.request.header.type === jute.OP_CODES.CREATE) {
                    pendingPacket.callback(
                        null,
                        pendingPacket.response.payload.path
                    );
                } else if (pendingPacket.request.header.type === jute.OP_CODES.DELETE) {
