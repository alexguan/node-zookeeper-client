...
ConnectionManager.prototype.close = function () {
    ...
    //console.log('queue closing request');

    self.queue(request, function (error, response) {
        //console.log('Get closed callback');

        if (error) {
            // Ignore
            //console.log('Got error when requesting close.' + error);
        }
    });
};
...
ConnectionManager.prototype.onSocketClosed = function (hasError) {
    ...
    if (retry) {
        ...
        //console.log('Changed state to closed.');
...
ConnectionManager.prototype.onSocketConnected = function () {
    ...
    if (!this.watcherManager.isEmpty()) {
        ...
        payload.dataWatches =
            this.watcherManager.getDataWatcherPaths().map(function (path) {
                return this.prependChroot(path);
            }, this);
        payload.existWatches =
            this.watcherManager.getExistenceWatcherPaths().map(function (path) {
                return this.prependChroot(path);
            }, this);
        payload.childWatches =
            this.watcherManager.getChildWatcherPaths().map(function (path) {
                return this.prependChroot(path);
            }, this);
...
ConnectionManager.prototype.onSocketData = function (buffer) {
    ...
    if (self.state === STATES.CONNECTING) {
        ...
        case jute.XID_NOTIFICATION:
            ...
            if (event.path) {
                event.path = self.removeChroot(event.path);
            }
        ...
        default:
            ...
            if (responseHeader.err === 0) {
                ...
                if (responsePayload) {
                    ...
                    // TODO: transaction result path.
                    if (responsePayload.path) {
                        responsePayload.path =
                            self.removeChroot(responsePayload.path);
                    }
                }
...
ConnectionManager.prototype.prependChroot = function (path) {
    if (!this.chrootPath) {
        return path;
    }
...
ConnectionManager.prototype.removeChroot = function (path) {
    if (!this.chrootPath) {
        return path;
    }

    if (path === this.chrootPath) {
        return '/';
    } else {
        return path.substring(this.chrootPath.length);
    }
};
...
ConnectionManager.prototype.queue = function (request, callback) {
    ...
    if (request.payload && request.payload.path) {
        request.payload.path = this.prependChroot(request.payload.path);
    }
    ...
    case STATES.CLOSING:
        if (request.header &&
            ...
            //console.log('pushing closing packet');
            this.packetQueue.push({
                request : request,
                callback : callback
            });
