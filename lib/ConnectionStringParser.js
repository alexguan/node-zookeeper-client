...
function ConnectionStringParser(connectionString) {
    ...
    hostList.filter(function (item) {
        ...
        if(parts.length > 2) { // IPv6 (RFC 3986) - [2001:db8:65:6c61:622e:6b72:010:3214]:4322
            // not use endsWith (ECMAScript 6)
            var ip_end_index = item.lastIndexOf(']');
            parts = [item.substring(1, ip_end_index)];

            // use port number
            if(ip_end_index != (item.length - 1)) {
                parts.push(item.substring(item.lastIndexOf(':') + 1));
            }
        }
