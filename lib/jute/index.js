...
Request.prototype.toBuffer = function () {
    ...
    //console.log('request buffer: 0x' + buffer.toString('hex'));
...
Response.prototype.fromBuffer = function (buffer) {
    ...
    if (this.header) {
        ...
        // TODO, change to use the error code constant.
        if (this.header.err !== 0) {
            // Skip payload de-serialization if we get a error response.
            return;
        }
    }
...
/**
 * Create a request object with given request operation code.
 *
 * @method createRequest
 * @param operationCode {number} The operation code of the request.
 */
function createRequest(operationCode) {
    if (typeof operationCode !== 'number') {
        throw new Error('operationCode must be a valid number.');
    }
